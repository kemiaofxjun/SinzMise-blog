...
function useRandomData(source, random = false) {
  ...
  watch(() => source, async () => {
    ...
    data.value = random ? Array.from(rawData.link_status).sort(() => Math.random() - 0.5) : rawData
  }, { immediate: true })
...
export const generatelink = (source, random = false) => {
  const { data } = useRandomData(source, random);
